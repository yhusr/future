...
from scripts.handle_conf import HandleYaml, uy
from scripts.handle_path import PERSONPATH, YAMLPATH


class HandleRe:
    ph = HandleYaml(PERSONPATH)
    @classmethod
    def str_regex(cls, strdata):
        """
        正则表达式替换内容
        :param strdata:
        :return:
        ...
        if re.search(r'{no_exist_num}', strdata):
            hm = HandleMysql()
            loan_id = int(hm.get_mysql_result(uy.open_yaml('mysql',
                                                                             'id_sql'))[0]['member_id'])+1
        ...
        if re.search(r'{loan_id_re}', strdata):
            hm = HandleMysql()
            invent_id = cls.ph.open_yaml('investor', 'userId')
            my_loan_id = getattr(cls, 'get_loan_id')
            res_data = re.sub(r'{loan_id_re}', str(my_loan_id), strdata)
        ...
        if re.search(r'{admin_phone}', strdata):
            admin_phone = str(cls.ph.open_yaml("admin", "userPhone"))
        ...
        if re.search(r'{load_id}', strdata):
            hm = HandleMysql()
            ph = HandleYaml(YAMLPATH)
            borrow_id = cls.ph.open_yaml('borrower', 'userId')
            member_id = hm.get_mysql_result(uy.open_yaml('mysql','loan_sql'),
                                            borrow_id)[0]['id']
